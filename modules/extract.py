...
def extract_fields(data):
    ...
    abstracts_retrieval_response = (
        data.get("abstracts-retrieval-response", {}) if data else {}
    )
    if not abstracts_retrieval_response:
        abstracts_retrieval_response = data
    item = (
        abstracts_retrieval_response.get("item", {})
        if abstracts_retrieval_response
        else {}
    )
    ...
    coredata = (
        abstracts_retrieval_response.get("coredata", {})
        if abstracts_retrieval_response
        else {}
    )
    sid = (
        coredata.get("dc:identifier", "").replace("SCOPUS_ID:", "", 1)
        if coredata.get("dc:identifier", "")
        else ""
    )
    authors = (
        abstracts_retrieval_response.get("authors", {}).get("author", [])
        if abstracts_retrieval_response
        else []
    )
    ...
    if isinstance(afflication, dict):
        afflication_city = (
            afflication.get("affiliation-city", "")
            if afflication.get("affiliation-city", "")
            else ""
        )
        afflication_country = (
            afflication.get("affiliation-country", "")
            if afflication.get("affiliation-country", "")
            else ""
        )
        affilname = (
            afflication.get("affilname", "") if afflication.get("affilname", "") else ""
        )
        ...
        for a in afflication:
            afflication_city = (
                a.get("affiliation-city", "") if a.get("affiliation-city", "") else ""
            )
            afflication_country = (
    ...
    subject_area = abstracts_retrieval_response.get("subject-areas", {}).get(
        "subject-area", []
    )
    sub = ""
    ...
    citation_title = (
        head.get("citation-title", "") if head.get("citation-title", "") else ""
    )
    abstracts = head.get("abstracts", "") if head.get("abstracts", "") else ""
    references = ref
    refcount = (
        bibliography.get("@refcount", "") if bibliography.get("@refcount", "") else 0
    )
    affiliations = aff
    citedby_count = (
        coredata.get("citedby-count", "") if coredata.get("citedby-count", "") else ""
    )
    ...
    description = (
        coredata.get("dc:description", "") if coredata.get("dc:description", "") else ""
    )
    publisher = (
        coredata.get("dc:publisher", "") if coredata.get("dc:publisher", "") else ""
    )
    cover_date = (
    ...
    aggregation_type = (
        ...
        if coredata.get("prism:aggregationType", "")
        else ""
    )
    publication_name = (
    ...
    language = (
        abstracts_retrieval_response.get("language", {})
        if abstracts_retrieval_response.get("language", {})
        else {}
    )
...
def extract(file_names):
    ...
    for file_name in file_names:
        ...
        try:
            ...
            logging.error(
                f"Unexpected error reading file: {file_name}, error: {str(e)}"
            )
            continue
