...
def do_store_in_mongo(task_instance):
    try:
        ...
        print("do_store_in_mongo: data: ", data)
...
def do_fetch_cursor_from_ids(task_instance):
    try:
        client = connect_to_mongo()
        file_path = "scopus_ids.json"

        # Open the JSON file and load its contents
        with open(file_path, "r") as json_file:
            data = json.load(json_file)
        return find_documents_by_ids(client, data)
    except Exception as e:
...
def do_store_result_from_training(task_instance):
    try:
        ...
        store_in_mongo(client, data, collection_name="scopus_class")
    except Exception as e:
